...
export class TodoService {
  ...
  public syncData(){
    syncClient.connect(WebSocket, this.url).then((data)=>{
      console.log("Mike test data", data)
    })
    .catch(e=>{
  ...
  private async sendItemsFromIndexedDb(){
    ...
    allItems.forEach((item: Todo) => {
     ...
     this.http.post<any>(this.url, item, this.httpOptions).subscribe(
      result =>{
        this.db.todos.delete(item.text).then(()=>{   //no need to delete items from index db
